...
def plot_selected_files_l3():
    ...
    if selected_file_indices:
        ...
        # Initialize arrays to store longitude, latitude,time and SSHA values
        merged_lons = []
        merged_lats = []
        merged_datetime = []
        merged_variable = []
        def plot_file(selected_file_info):
            ...
            # Append longitude, latitude,time and SSHA values to the merged arrays
...
def filter_files_by_coordinates_l3(folder_path, min_lat, max_lat,min_lon,max_lon,start_time,end_time,min_variable,max_variable,selected_variable, pbar=None):
        ...
        if file_name.endswith('.nc'):
            ...
            try:
                ...
                with xr.open_dataset(file_name) as dataset:
                    ...
                    # Extract latitude and longitude and time coordinates from the dataset
                    ...
                    if len(common_indices) != 0:
                        ...
                        # Check if there are non-NaN SSHA values within the specified latitude and longitude and time ranges
...
def button_3_clicked_l2():
    ...
    folder_path = choose_folder_l3()  # Choose the folder path through file management system
    ...
    filtered_files = filter_files_by_coordinates_l3(folder_path, min_lat,max_lat,min_lon,max_lon, start_time,end_time,min_variable, max_variable, selected_variable, pbar)
...
def plot_data_l2(lons, lats, variable_data,min_lat, max_lat, min_lon, max_lon, min_variable,max_variable):
    ...
    if os.path.exists(coastline_shapefile):
        ...
        # Plot SSHA values within the specified latitude and longitude and time ranges
...
def update_image_l3(ind):
    ...
    ind += 1
    root.after(100, update_image_l3, ind % gif_image.n_frames)
...
default_start_date = '2023-10-10' # set default start date
...
default_end_date = '2023-10-11' # set default end date
...
variable_dropdown = ttk.Combobox(button_frame_l2, values=['mdt','ssha','ssha_noiseless'])
variable_dropdown.set("ssha") # setting of the default value
variable_dropdown.grid(row=0, column=1)

# Plot button
plot_variable = tk.Button(button_frame_l2, text="Plot the selected variable", command=plot_selected_files_l3, font=button_font, bg=button_bg_color)
